    ...
    if (mysqli_connect_errno($con)){
        ...
        if($State == "매니저미지정"){
            ...
            if (!mysqli_query($con,$query)){
                ...
                error_log("알람 activate 상태를 0으로 변경하기 - 읽음 표시 개념임");
                ...
                if (!mysqli_query($con,$query)){
                    ...
                    error_log("서비스 매니저 이름 매니저미지정으로 변경 완료 + 읽음 표시까지 완료 ");
                    echo json_encode(array( "status" => "true","message" => "서비스 매니저 이름 매니저미지정으로 변경 완료 + 읽음 표시까지 완료 ") , JSON_UNESCAPED_UNICODE);
